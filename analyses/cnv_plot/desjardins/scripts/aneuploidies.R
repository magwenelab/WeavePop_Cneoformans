...
high_chrom_depth <- depth_by_chrom_good %>%
...
cnv_percent <- cnv %>%
    ...
    summarise(total_cnv_size = sum(region_size),
                ...
                first = min(start),
                last = max(end),
                mean_smooth_depth = mean(smooth_depth)) %>%
...
#### Join CNV percent with duplications detected by normalized chromosome depth ####
...
duplications <- left_join(high_chrom_depth, cnv_percent, by = c("lineage", "dataset", "sample", "strain", "chromosome", "source", "accession", "samples_in_dataset_lineage", "samples_in_lineage", "total_samples"))%>%
    droplevels()

# depth_by_chrom <- depth_by_chrom_good %>%
#     select(sample, accession, norm_chrom_median)

# # Filter by normalized chromosome median depth
# stats <- left_join(cnv_percent, depth_by_chrom, by = c("sample", "accession" )) %>%
#     filter(norm_chrom_median > 1.55)%>%
#     arrange(percent_cnv_size) %>%
#     mutate(chrom_sample = paste(chromosome, sample, sep="_"))
# stats$chrom_sample <- factor(stats$chrom_sample, levels = stats$chrom_sample)
# stats <- pivot_longer(stats, cols = c("percent_cnv_size", "percent_size_covered", "norm_chrom_median", "mean_smooth_depth", "n_cnvs"), names_to = "variable", values_to = "value")

# # plot stats

# p <- ggplot(stats, aes(x = chrom_sample, y = value, color = variable))+
#         geom_point()+
#         facet_grid( scales="free", rows= vars(variable))+
#         theme(axis.text.x = element_text(angle = 90, hjust = 1))
# p

# # Filter by percent cnv size
# stats <- left_join(cnv_percent, depth_by_chrom, by = c("sample", "accession" )) %>%
#     filter(percent_cnv_size > 80)%>%
#     arrange(percent_cnv_size) %>%
#     mutate(chrom_sample = paste(chromosome, sample, sep="_"))
# stats$chrom_sample <- factor(stats$chrom_sample, levels = stats$chrom_sample)
# stats <- pivot_longer(stats, cols = c("percent_cnv_size", "percent_size_covered", "norm_chrom_median", "mean_smooth_depth", "n_cnvs"), names_to = "variable", values_to = "value")
...
duplicated <- duplications %>%
    select(dataset, lineage, sample, strain, source, chromosome, norm_chrom_median, percent_cnv_size, n_cnvs)
write_tsv(duplicated, "../results/duplicated.tsv")
...
# After manual inspection of plots (see script plots_duplicated.xsh)
...
write_tsv(duplicated_full_strain, "../results/full_duplicated.tsv")
...
dup_sample <- duplications_full%>%
    ...
    summarise(n_chroms = n_distinct(chromosome), chromosomes = paste(chromosome, collapse = ", ")) %>%
    arrange(desc(n_chroms))
write_tsv(dup_sample, "../results/full_dup_sample.tsv")
...
write_tsv(dup_dataset_lineage_chromosome, "../results/full_dup_dataset_lineage_chromosome.tsv")
...
write_tsv(dup_lineage_chromosome, "../results/full_dup_lineage_chromosome.tsv")
...
write_tsv(dup_lineage_dataset, "../results/full_dup_lineage_dataset.tsv")
...
write_tsv(dup_chromosome, "../results/full_dup_chromosome.tsv")
