...
duplications <- select(duplications, sample, ploidy)%>%
    distinct()
duplications$ploidy <- factor(duplications$ploidy, levels = c("haploid", "diploid", "triploid", "tetraploid"))
```
...
components <- bind_rows(components_list)%>%
    left_join(duplications, by = "sample")%>%
    arrange(ploidy) 
```
```{r}
components$sample <- factor(components$sample, levels = unique(components$sample))
```
...
ggplot(large_components)+
    ...
    geom_point(aes(x = mean, y = sample, color = weight))+
    facet_wrap(~ploidy, scales = "free_y")+
    scale_color_viridis_c(direction = -1)+
    theme_classic()+
    theme(panel.grid.major = element_line(color = "gray", linetype = "dashed"))+
    labs(title = "Mean of large components per sample",
