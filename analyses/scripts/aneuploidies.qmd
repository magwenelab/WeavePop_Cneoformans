...
n_lineage_source_path <- "results/figs/barplot_lineage_source.svg"
```
...
* absent: [zero_threshold, zero_threshold]
...
chrom_metrics$category <- factor(chrom_metrics$category, levels = c("full", "partial", "small", "absent"))
```

Remove single-copy observations and join with metadata
```{r}
...
pivot_wider(dup_sublineage, names_from = chromosome, values_from = n_samples, values_fill = "")
```
...
dup_lineage_source  %>%
        filter(type == "Aneuploid")%>%
        select(lineage, source, n_samples, percent_samples, samples_in_lineage_source)
```

```{r}
category_colors <- brewer.pal(11, "BrBG")[c(4, 2, 8 ,10)]
names(category_colors) <- c("Environmental Euploid", "Environmental Aneuploid","Clinical Euploid","Clinical Aneuploid")
```
...
#| fig-width: 13
p <- ggplot() +
        geom_col(data = dup_lineage_source, aes(x = lineage, y = percent_samples, fill = category), color = NA, position = "stack")+
        geom_text(data = dup_lineage_source, aes(x = lineage, y = percent_samples +2, label = dup_samples), position = "identity", size = 8)+
        scale_fill_manual(values = category_colors)+
        geom_text(data = dup_lineage_source, aes(x = lineage, y = 108, label = paste("n = ", samples_in_lineage)), size = 8)+
...
```{r}
ggsave(n_lineage_source_path, p, height = 5.3, width = 14, units = "in", dpi = 900)
```
