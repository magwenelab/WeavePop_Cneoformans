...
do
    grep $set analyses/results/tables/ploidy_from_plots.tsv |\
    cut -f 1 |\
    while read line
    do
        quarto render analyses/scripts/ploidy_depth.qmd  -P sample_id:\"${line}\" -P dataset:\"${set}\" -P max_iterations:10000 -P threshold_weight:0.2 -P threshold_norm_depth:5
