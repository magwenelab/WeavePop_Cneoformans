...
chrom_names_path <- 
  paste("../Crypto_",
  params$dataset,
  "/config/chromosomes.csv", sep ="")
log_wg_path <- 
  paste("notebooks/scripts/ploidy_depth/logs/whole_genome_", params$sample_id, ".txt")
log_chrom_path <- 
  paste("notebooks/scripts/ploidy_depth/logs/chromosomes_", params$sample_id, ".txt")
components_path <- 
  paste0("results/tables/components_", params$sample_id, ".tsv")
```
...
```{r, output = FALSE}
chrom_names <- read_csv(chrom_names_path)
...
Thresold of normalized depth:
```{r}
print(params$threshold_norm_depth)
```
```{r}
...
Fitting GMM with max_iterations:
```{r}
...
sink(log_wg_path)
print("--------------Whole Genome Gaussian Mixture Model Analysis --------------")
...
```
```{r, error = TRUE, output = FALSE }
sink(log_chrom_path, append=TRUE)
print("--------------Per Chromosome Gaussian Mixture Model Analysis --------------")
for (chrom in levels(windows_filtered$chromosome)){
  for (component in 2:5) {
    component_name <- paste("k", component, sep = "")
    ...
    print(paste("Analyzing", chrom, "with k=", component))
    ...
    models_list[[chrom]][[component_name]] <- normalmixEM(
      ...
      print(paste("Error in chromosome", chrom, "with", component, "components:", e$message))
    })
    # Calculate BIC
...
for (chrom in levels(windows_filtered$chromosome)) {
  ...
  for (i in 1:length(model$mu)) {
    ...
    temp_data <- data.frame(x = x, y = y, 
                          component = as.factor(i),
                          mean = model$mu[i], 
                          variance = model$sigma[i], 
                          weight = model$lambda[i])
...
components <- combined_data %>%
  select(chromosome, k, component, mean, weight, variance)%>%
  distinct()%>%
...
ggplot(components)+
  ...
  theme_classic()
```
Saving file with the parameters of all components of each chromosome:
```{r}
write_tsv(components, components_path)
```
Filtering out components with a weight less than:
```{r}
print(params$threshold_weight)
```
```{r}
...
ggplot(large_components) +
  ...
  geom_vline(xintercept = seq(0,max(large_components$mean), by = 1),
...
ggplot(large_components) +
  geom_point(aes(x = mean, y = weight, color = chromosome)) +
  geom_text_repel(aes(x = mean, y = weight, color = chromosome, label = chromosome), max.overlaps = Inf) +
  geom_vline(xintercept = seq(0,max(large_components$mean), by = 1/2),
            color = "black",
            linetype = "solid") + 
  scale_x_continuous(breaks = ploidy$diploid,
...
ggplot(large_components) +
  geom_point(aes(x = mean, y = weight, color = chromosome)) +
  geom_text_repel(aes(x = mean, y = weight, color = chromosome, label = chromosome), max.overlaps = Inf) +
  geom_vline(xintercept = seq(0,max(large_components$mean), by = 1/4), color = "black", linetype = "solid") +    scale_x_continuous(breaks = ploidy$tetraploid,
