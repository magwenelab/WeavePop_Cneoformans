...
ggsave(ashton_tree_unrooted_plot, 
...
```{r, echo=FALSE, warning=FALSE}
ggsave(ashton_tree_rooted_plot, p, height = 10, width = 10, units = "in", dpi = 600)
```
...
#| fig-width: 7
#| fig-align: center
pr <- ggtree(ashton_tree, layout = "rectangular", size =0.2)+
            geom_tippoint(aes(color= sublineage[match(label, rownames(sublineage)), "vni_subdivision"]),
                              size= 1)+
            scale_color_manual(values = sublineage_colors, 
                               name="VNI Sublineage", 
            ...
            geom_treescale(x=0, y=0, width=0.01, fontsize =5, offset=1)+
            labs(title = "Rooted tree of Ashton dataset")

m1 <- gheatmap(pr, dataset, width=.02, colnames=FALSE, offset=.015) +
