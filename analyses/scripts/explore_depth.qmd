...
# Output
cnv_out_path <- 
    "results/tables/haploid_samples_with_cnv.tsv"
ploidy_out_path <- 
...
cnv_chroms <- read.delim(
    ...
    select(sample, chromosome,cnv, chrom_norm_depth, coverage_percent)%>%
    pivot_wider(
        names_from = cnv,
        values_from = coverage_percent)%>%
    select(sample, chromosome,chrom_norm_depth,
        coverage_percent_duplication = duplication)

```

```{r}
ggplot(cnv_chroms, aes(x = coverage_percent_duplication, y = chrom_norm_depth)) +
...
cnv_chroms_subset <- filter(cnv_chroms, 
    ...
    chrom_norm_depth >= depth_threshold)%>%
...
print_plots <- function(df){
    for (sample_id in unique(df$sample)){
        ...
        metrics <- df %>% 
            filter(sample == sample_id)%>%
            select(sample, chromosome, chrom_norm_depth, coverage_percent_duplication)%>%
            as.data.frame()
...
```{r}
ploidy_df <- data.frame(sample = non_haploid, ploidy = "non_haploid")
```
...
```{r}
ggplot(cnv_chroms_ploidy, aes(x = coverage_percent_duplication, y = chrom_norm_depth, color = ploidy)) +
        geom_hline(yintercept = c(1, 2), color = "black", linetype = "solid") +
...
desjardins_non_haploids <- cnv_chroms_ploidy %>%
    filter(dataset == "Desjardins", ploidy == "non_haploid")%>%
    select(sample)%>%
    distinct()%>%
    pull(sample)
ashton_non_haploids <- cnv_chroms_ploidy %>%
    filter(dataset == "Ashton", ploidy == "non_haploid")%>%
    select(sample)%>%
    distinct()%>%
    pull(sample)
```
...
### Write table of haploid samples with detected CNVs

```{r}
low_mapq <- c("ERS542595",
                "ERS542410",
                "ERS542415")
```

```{r}
cnv_haploids <- cnv_chroms_subset %>%
    filter(!sample %in% non_haploid, !sample %in% low_mapq)%>%
    select(sample, dataset, lineage)%>%
    arrange(desc(dataset),lineage)%>%
    distinct()
write.table(cnv_haploids, cnv_out_path,
    row.names = FALSE,
    col.names = FALSE,
    quote = FALSE,
    sep = "\t")
