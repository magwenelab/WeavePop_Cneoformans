...
## Get highly heterozygous samples

```{r}
num_snps <- read_csv("../data/processed/count_hetero/summary.csv")
```

```{r}
p <- ggplot(num_snps)+
    geom_histogram(aes(n_hetero), bins =387)
p
```

```{r}
num_snps <-num_snps %>%
    filter(n_hetero <10000)
```
```{r}
p <- ggplot(num_snps)+
    geom_histogram(aes(n_hetero), bins =387)
...
```{r}
head(arrange(num_snps,desc(n_hetero)))
```

## One Desjardins sample with high heterozygosity Bt65
...
#vcf <- read.vcfR("/FastData/czirion/Crypto_Diversity_Pipeline/analyses/ploidy/data/processed/haploid_freebayes/PMY3519.snps.raw.vcf")
```
